    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`interactionuserImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="interactionuserImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="interactionuserImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useInteractionUserStore());
const { createManyInteractionUser } = useInteractionUserStore();
...
const interactionuserInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      interactionuserInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      interactionuserInsertData.value = interactionuserInsertData.value.map(
        (item: any) => {
          let res: any = {};
          for (let key in rest) {
      ...
      await createManyInteractionUser(interactionuserInsertData.value);
