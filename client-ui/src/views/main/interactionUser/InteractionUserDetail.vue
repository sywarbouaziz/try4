  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="interactionuser">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(interactionuser)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="interactionuserDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { interactionuser } = storeToRefs(useInteractionUserStore());
const { getInteractionUserById } = useInteractionUserStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getInteractionUserById(id);
  setCurrentPageBreadcrumbs("interactionusers", ["detail"]);
});
</script>
<style>
.bottom {
