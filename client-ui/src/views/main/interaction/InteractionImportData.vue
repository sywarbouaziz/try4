    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`interactionImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="interactionImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="interactionImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useInteractionStore());
const { createManyInteraction } = useInteractionStore();
const router = useRouter();
import { useAuthStore } from "@/store/useAuth";
const { currentUser } = storeToRefs(useAuthStore());

const initialState = { interactionType: "", userId: "", annonceId: "" };
...
const interactionInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      interactionInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      interactionInsertData.value = interactionInsertData.value.map(
        (item: any) => {
          let res: any = {};
          for (let key in rest) {
      ...
      await createManyInteraction(interactionInsertData.value);
