  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="interaction">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(interaction)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="interactionDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { interaction } = storeToRefs(useInteractionStore());
const { getInteractionById } = useInteractionStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getInteractionById(id);
  setCurrentPageBreadcrumbs("interactions", ["detail"]);
});
</script>
<style>
.bottom {
