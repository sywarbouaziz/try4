    ...
    >
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`annonceList_${annonceList[scope.$index].id}`"
                :to="`/${currentUser.role}/${entityPluralName}/${
                  annonceList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`annonceList${fieldName}EditLink_${
                annonceList[scope.$index].id
              }`"
            ...
            <el-button
              :data-test="`annonceList${fieldName}DeleteBtn_${
                annonceList[scope.$index].id
              }`"
              @click="
                () => handleOpenConfirmModal(annonceList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === annonceList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(annonceList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper">
        <Pagination
          :itemsPerPage="annoncePagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(annoncePagination.skip / annoncePagination.take) + 1
          "
          :current-page-change="currentPageChange"
          :total="annoncePagination.total._count?._all"
...
const entityPluralName = "annonces";
...
const { annonceList, annoncePagination, annonceExcelFile } = storeToRefs(
  useAnnonceStore()
);
const {
  fetchAnnonces,
  deleteAnnonce,
  fetchDataExcelAnnonces,
  softDeleteAnnonce,
} = useAnnonceStore();

async function dataToExcel() {
  await fetchDataExcelAnnonces();
  const workbook = XLSX.read(annonceExcelFile.value.toString());
  XLSX.writeFile(workbook, "annonceData.xlsx");
}
const deleteLoading = ref<boolean>(false);
const handleRemoveEntity = async (id: string) => {
  deleteLoading.value = true;
  await softDeleteAnnonce(id);
...
const currentPageChange = async (val: number) => {
  await fetchAnnonces({
    skip: (val - 1) * annoncePagination.value.take,
    take: annoncePagination.value.take,
  });
};
onMounted(async () => {
