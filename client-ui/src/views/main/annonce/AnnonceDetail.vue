  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="annonce">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(annonce)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="annonceDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { annonce } = storeToRefs(useAnnonceStore());
const { getAnnonceById } = useAnnonceStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getAnnonceById(id);
  setCurrentPageBreadcrumbs("annonces", ["detail"]);
});
</script>
<style>
.bottom {
